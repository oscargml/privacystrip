...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] text-slate-900 font-sans selection:bg-indigo-100 selection:text-indigo-700">
      ...
      <main className="max-w-5xl mx-auto px-6 pb-16">
        ...
        <section id="how" className="mt-24 border-t border-slate-100 pt-16">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <InfoCard
              ...
              title="Wipe GPS Data"
              description="Remove precise coordinates that reveal exactly where a photo was taken — city, street, or even your home address."
            />
            <InfoCard
              icon={<User className="w-5 h-5 text-indigo-600" />}
              title="Clean PDF Authors"
              description="Strip metadata that leaks your name, job title, software used, and company info from Word-to-PDF exports."
            />
            <InfoCard
              icon={<Settings className="w-5 h-5 text-indigo-600" />}
              title="Erase Device Info"
              description="Hide camera model, lens settings, firmware version, and software tags before posting images online."
            />
          </div>
        </section>
        ...
        <section id="faq" className="mt-20 border-t border-slate-100 pt-16 space-y-8 max-w-2xl mx-auto">
          ...
          <Faq
            ...
            a="No. Every operation runs entirely inside your web browser using JavaScript. Your files never leave your device and are never sent to any server."
          />
          <Faq
            q="How do I remove GPS location from an iPhone photo online free?"
            a="Simply drag your JPEG photo into the box above, click Clean File, and download the result. PrivacyStrip surgically removes the GPS EXIF segment without re-compressing the image."
          />
          <Faq
            q="How do I strip the author name from a PDF without uploading it?"
            a="Drop the PDF file into the tool. PrivacyStrip reads the XMP/DocInfo metadata block with pdf-lib and rewrites the document with all author, creator, and producer fields cleared."
          />
          <Faq
            q="Will cleaning metadata reduce image quality?"
            a="For JPEG files, we remove the EXIF header directly without touching pixel data, so quality is preserved at 100%. PNG and WebP files are redrawn via canvas at 92% quality."
          />
        </section>
      ...
      {/* Footer */}
      <footer className="max-w-5xl mx-auto px-6 py-10 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4">
